...
# ---- NEW settings keyboard (used in wizard) ----
...
# ---------- ADMIN HELP BUTTONS ----------
# These buttons now call the appropriate wizard. The three settings buttons now use the settings wizard.
...
def admin_help_click(m):
    ...
    # Handle broadcast and clearlogs directly
    if text == "📢Broadcast":
    ...
    # For Link, BotToken, AIKey -> use the settings wizard
...
# ---------- ADMIN WIZARD (for search, add, delete, block, unblock, clearlogs) ----------
...
# ---------- REWRITTEN SETTINGS WIZARD (unified for all six buttons) ----------
SETTING_MAP = {
    'link': 'channel_link',
    'botapi': 'bot_token',
    'aiapi': 'ai_api_key'
...
def start_settings_wizard(uid, setting, chat_id):
    ...
    clear_user_sessions(uid)  # clear any ongoing session
    settings_sessions[uid] = {
        'setting': setting,
        'step': 1,          # 1: ask for value, 2: ask for ID (only for link), 3: confirmation
        'data': {},
        'chat_id': chat_id
    }
...
def show_settings_confirmation(uid):
    """Step 3: show the summary with Done/Cancel/Back buttons."""
    sess = settings_sessions.get(uid)
    if not sess:
        return
    setting = sess['setting']
    ...
    kb = settings_confirm_keyboard()  # ✅ Done | ❌ Cancel | 🍃back
    bot.send_message(sess['chat_id'], text, parse_mode='Markdown', reply_markup=kb)
    sess['step'] = 3

# ---- SINGLE HANDLER FOR SETTINGS WIZARD ----
    ...
    if setting == 'link':
        if step == 1:
            ...
            try:
                cid = int(text)
                sess['data']['id'] = cid
                show_settings_confirmation(uid)  # will set step=3
            except ValueError:
        ...
        if step == 1:
            ...
            show_settings_confirmation(uid)  # set step=3
        else:
            bot.reply_to(m, "❌ Unexpected step. Please restart the wizard.")
            del settings_sessions[uid]
    else:
