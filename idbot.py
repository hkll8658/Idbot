...
# ---------- ADMIN HELP BUTTONS ----------
# Note: "🍃back" and "✅ Done", "❌ Cancel" are intentionally NOT here – they are handled by their respective wizards.
...
def admin_wizard_handler(m):
    ...
    if text == "🍃back":
        if step == 1:
            ...
            sess['step'] = step - 1
            if cmd == 'link' and step == 2:
                bot.send_message(m.chat.id,
                                 "🔗 *Channel Link Update - Step 1/2*\n\nPlease send the new channel link (invite URL).\nExample: `https://t.me/+abc123`",
                                 parse_mode='Markdown', reply_markup=wizard_input_keyboard())
            elif cmd == 'botapi' and step == 2:
                bot.send_message(m.chat.id,
                                 "🍶 *Bot Token Update*\n\nPlease send the new bot API token.\nExample: `123456:ABC-DEF...`",
                                 parse_mode='Markdown', reply_markup=wizard_input_keyboard())
            elif cmd == 'aiapi' and step == 2:
                bot.send_message(m.chat.id,
                                 "🦞 *AI API Key Update*\n\nPlease send the new AI API key.\nExample: `sk-...`",
                                 parse_mode='Markdown', reply_markup=wizard_input_keyboard())
            else:
    ...
    # INPUT HANDLING for admin wizard (same as before)
    if cmd == 'link':
        if step == 1:
...
# ---------- SETTINGS WIZARD (FIXED – Now works exactly like broadcast) ----------
SETTING_MAP = {
    'link': 'channel_link',
    'botapi': 'bot_token',
    'aiapi': 'ai_api_key'
...
# Single handler for all settings wizard messages
...
def settings_wizard_handler(m):
    ...
    if text == "🍃back":
        if step == 3:
            # From confirmation, go back to step 2 (or step 1 for botapi/aiapi)
            if setting == 'link':
                sess['step'] = 2
