...
# ---------- NEW: Exit AI Mode Button Handler ----------
...
# ---------- ADMIN HELP BUTTONS ----------
# Removed "🍃back", "✅ Done", "❌ Cancel" from this list to avoid conflict with settings wizard
...
# ---------- SETTINGS WIZARD (REWRITTEN – like broadcast) ----------
SETTING_MAP = {
    'link': 'channel_link',
    'botapi': 'bot_token',
    'aiapi': 'ai_api_key'
...
# ---- SINGLE HANDLER FOR SETTINGS WIZARD (like broadcast) ----
...
def settings_wizard_handler(m):
    ...
    # ---- DONE (placeholder – will add database update later) ----
    if text == "✅ Done":
        # For now, just confirm and go back
        bot.reply_to(m, "✅ Update confirmed! (Database update will be added next.)")
        del settings_sessions[uid]
        send_managebot_menu(m.chat.id)
        return
    ...
    if setting == 'link':
        if step == 1:
            ...
            try:
                cid = int(text)
                sess['data']['id'] = cid
                send_settings_confirm(uid)  # will set step=3
            except ValueError:
        ...
        if step == 1:
            ...
            send_settings_confirm(uid)  # set step=3
        else:
            bot.reply_to(m, "❌ Unexpected step. Please restart the wizard.")
            del settings_sessions[uid]
    else:
