...
# ---- inline keyboard for settings confirmation ----
def settings_inline_keyboard():
    kb = InlineKeyboardMarkup(row_width=2)
    kb.row(
        InlineKeyboardButton("✅ Confirm", callback_data="settings_done"),
...
def admin_help_click(m):
    ...
    # Settings buttons – use the settings wizard
...
def admin_wizard_handler(m):
    ...
    if text == "🍃back":
        if step == 1:
            ...
            # For admin wizard, there is only one step (input) and then confirmation, so back just cancels.
            del admin_cmd_sessions[uid]
            bot.send_message(m.chat.id,
                             f"🥀 Hello, *{m.from_user.first_name}* welcome back to admin👋",
                             parse_mode='Markdown', reply_markup=admin_main_reply_keyboard())
...
# ---------- REWRITTEN SETTINGS WIZARD (using inline keyboard for confirmation) ----------
SETTING_MAP = {
    'link': 'channel_link',
    'botapi': 'bot_token',
    'aiapi': 'ai_api_key'
...
def send_managebot_menu(chat_id):
    """Send the managebot menu (used after settings wizard ends)."""
    bot.send_message(chat_id,
                     "🧞welcome back to bot management system.\n🎭Select management button's🥀\n💬Owner Contact : @OWNERHIMANSHU",
                     parse_mode='Markdown', reply_markup=manage_bot_reply_keyboard())

def start_settings_wizard(uid, setting, chat_id):
    """Start the settings wizard (called from any of the six buttons)."""
    clear_user_sessions(uid)
    settings_sessions[uid] = {
        'setting': setting,
        'step': 1,          # 1: ask for value, 2: ask for ID (only for link), 3: confirmation (inline)
        'data': {},
        'chat_id': chat_id
    }
...
def send_settings_step1(uid):
    """Step 1: ask for the new value (link, token, or key)."""
...
def send_settings_step2(uid):
    """Step 2: ask for channel ID (only for link setting)."""
    sess = settings_sessions.get(uid)
    if not sess:
        return
    if sess['setting'] == 'link':
        ...
        # For botapi and aiapi, skip step 2 and go directly to confirmation
        show_settings_confirmation(uid)

def show_settings_confirmation(uid):
    """Step 3: show the summary with inline keyboard (Done/Cancel/Back)."""
    sess = settings_sessions.get(uid)
    if not sess:
        return
    setting = sess['setting']
...
# ---- Message handler for settings wizard (step1 & step2) ----
@bot.message_handler(func=lambda m: m.from_user.id in settings_sessions)
def settings_wizard_message_handler(m):
    uid = m.from_user.id
    sess = settings_sessions.get(uid)
    if not sess:
        return
    ...
    if text == "🍃back":
        if step == 3:
            # From confirmation, go back to step 2 (or step 1 for non-link)
            if setting == 'link':
                sess['step'] = 2
            ...
            # From step 2 (only link), go back to step 1
            sess['step'] = 1
            sess['data'].pop('link', None)
            send_settings_step1(uid)
        else:
            # step == 1: go back to managebot
            del settings_sessions[uid]
            send_managebot_menu(m.chat.id)
        return

    # ---- INPUT HANDLING (step 1 & 2) ----
    if setting == 'link':
        if step == 1:
            ...
            try:
                cid = int(text)
                sess['data']['id'] = cid
                show_settings_confirmation(uid)  # will set step=3 and send inline keyboard
            except ValueError:
        ...
        if step == 1:
            ...
            show_settings_confirmation(uid)  # set step=3 and send inline keyboard
        else:
            bot.reply_to(m, "❌ Unexpected step. Please restart the wizard.")
            del settings_sessions[uid]
    else:
...
# ---- Callback handler for settings confirmation ----
...
def settings_callback_handler(call):
    ...
    action = call.data.split("_")[1]  # "done", "cancel", "back"
    setting = sess['setting']
    data = sess['data']
    ...
    if action == "cancel":
        ...
        send_managebot_menu(call.message.chat.id)
        try:
    ...
    if action == "back":
        ...
        if setting == 'link':
            ...
            sess['data'].pop('key', None)
            send_settings_step1(uid)
        bot.answer_callback_query(call.id, "🔙 Going back...")
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
        except:
            pass
    ...
    if action == "done":
        ...
        if setting == 'link':
            ...
            if link and cid:
                ...
                bot.send_message(call.message.chat.id, "✅ Channel link and ID updated successfully!")
            else:
            ...
            if token:
                ...
                bot.answer_callback_query(call.id, "✅ Bot token updated! Restarting...")
                bot.send_message(call.message.chat.id, "✅ Bot token updated! Restarting in 3 seconds...")
                del settings_sessions[uid]
                time.sleep(1)
                restart_bot()
                return
            else:
                bot.answer_callback_query(call.id, "❌ Missing token.")
        elif setting == 'aiapi':
            key = data.get('key')
            if key:
                db.set_config('ai_api_key', key)
                db.add_log('aiapi', f"Updated to: {key}")
                bot.answer_callback_query(call.id, "✅ AI API key updated!")
                bot.send_message(call.message.chat.id, "✅ AI API key updated successfully!")
            else:
                bot.answer_callback_query(call.id, "❌ Missing key.")
        else:
            bot.answer_callback_query(call.id, "❌ Unknown setting.")

        # Clean up and go to managebot
        del settings_sessions[uid]
        send_managebot_menu(call.message.chat.id)
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
