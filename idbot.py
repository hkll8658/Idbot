...
# ---------- ADMIN HELP BUTTONS ----------
# For settings updates (Link, BotToken, AIKey), we now use the settings wizard.
# Other functions still use the admin wizard.
...
def admin_help_click(m):
    ...
    # For Link, BotToken, AIKey -> use the settings wizard (same as managebot)
...
# ---------- SETTINGS WIZARD (Unified - works for both managebot and admin help buttons) ----------
SETTING_MAP = {
    'link': 'channel_link',
    'botapi': 'bot_token',
    'aiapi': 'ai_api_key'
...
def cmd_link(m):
    ...
    if len(parts) >= 3:
        ...
        start_settings_wizard(m.from_user.id, 'link', m.chat.id)  # Use settings wizard
...
def cmd_botapi(m):
    ...
    if len(parts) >= 2:
        ...
        start_settings_wizard(m.from_user.id, 'botapi', m.chat.id)  # Use settings wizard
...
def cmd_aiapi(m):
    ...
    if len(parts) >= 2:
        ...
        start_settings_wizard(m.from_user.id, 'aiapi', m.chat.id)  # Use settings wizard
...
# ---------- GENERIC BACK HANDLER (placed at the end) ----------
